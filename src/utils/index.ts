...
<table cellpadding="0" cellspacing="0" border="0" style="font-family: Arial, sans-serif; max-width: 480px; padding: 12px 0; background-color: #ffffff; border-bottom: 1px solid #e5e2e1;">
  <tr>
    ...
    case 'premium-boxed': {
      ...
      const iconClass = isAnimated ? 'anim-pulse' : '';
      
      if (sig.website) {
        socialIconCells.push(`<td class="layout-web-icon" style="padding:0 4px 0 0"><a href="${formatUrl(sig.website)}" target="_blank"><img class="${iconClass}" alt="Website" src="${baseUrl}/domain--v1.png" width="32" style="border-radius: 4px; display: block;"></a></td>`);
      }
      if (sig.socials.instagram) {
        socialIconCells.push(`<td class="layout-insta-icon" style="padding:0 4px 0 0"><a href="${formatUrl(sig.socials.instagram)}" target="_blank"><img class="${iconClass}" alt="Instagram" src="${baseUrl}/instagram-new--v1.png" width="32" style="border-radius: 4px; display: block;"></a></td>`);
      }
      if (sig.socials.linkedin) {
        socialIconCells.push(`<td class="layout-linkedin-icon" style="padding:0 4px 0 0"><a href="${formatUrl(sig.socials.linkedin)}" target="_blank"><img class="${iconClass}" alt="LinkedIn" src="${baseUrl}/linkedin.png" width="32" style="border-radius: 4px; display: block;"></a></td>`);
      }
      if (sig.socials.facebook) {
        socialIconCells.push(`<td class="layout-facebook-icon" style="padding:0 4px 0 0"><a href="${formatUrl(sig.socials.facebook)}" target="_blank"><img class="${iconClass}" alt="Facebook" src="${baseUrl}/facebook-new.png" width="32" style="border-radius: 4px; display: block;"></a></td>`);
      }
      if (sig.socials.youtube) {
        socialIconCells.push(`<td class="layout-youtube-icon" style="padding:0 4px 0 0"><a href="${formatUrl(sig.socials.youtube)}" target="_blank"><img class="${iconClass}" alt="YouTube" src="${baseUrl}/youtube-play.png" width="32" style="border-radius: 4px; display: block;"></a></td>`);
      }
      if (sig.socials.twitter) {
        socialIconCells.push(`<td class="layout-twitter-icon" style="padding:0 4px 0 0"><a href="${formatUrl(sig.socials.twitter)}" target="_blank"><img class="${iconClass}" alt="Twitter" src="${baseUrl}/twitter--v1.png" width="32" style="border-radius: 4px; display: block;"></a></td>`);
      }

      const hasSocialIcons = socialIconCells.length > 0;
      let socialsRowHTML = '';
      if (hasSocialIcons) {
        socialsRowHTML = `<tr><td valign="top" align="left" class="layout_border layout_socialicon_border" style="border-collapse:separate; border-radius:5px; border-width: 1px; border-color:#cccccc; border-style: solid; padding:8px; display: compact;"><table border="0" cellspacing="0" cellpadding="0"><tbody><tr>${socialIconCells.join('')}</tr></tbody></table></td></tr>`;
      }
